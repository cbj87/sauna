...
- Thread safety: `_booking_lock` (booking creation), `_login_lock` (rate limiting), `_stats_lock` (Harvia stats), `_activation_lock` (pending-activation generation counter)
- **Sauna start is two-phase**: `turn_on()` needs 20–45s for the device to commit `onTime` before activating, so `/api/sauna/on`, preheat, extend, and set run `turn_on_stage()` synchronously (fast, proves reachability) and finish the commit-wait + `active=1` via `_activate_sauna_async()` in a background thread. Responses include `pending: true`; the frontend shows "Sauna is starting…" and polls live status until `active=1`. Any turn-off bumps the activation generation counter, so a stale background activation can never re-ignite the sauna after an off.
- **A start is optimistic until verified — and the verification is mandatory.** A Harvia `set_state` mutation succeeds whenever the *cloud* accepts the write; it says nothing about whether the heater got it. So the response, the auto-created booking and the Live Activity all assume success, and two guards keep that assumption honest:
  1. `turn_on_stage()` calls `assert_online()` first and raises `SaunaOffline` (→ HTTP 503) if `reported.online` is false — no phantom booking gets created.
  2. `turn_on_activate()` polls `reported.active` for 30s after sending `active=1`, retries the activate once, then raises `SaunaStartUnconfirmed`.
  On either failure `_activate_sauna_async()` unwinds everything it optimistically created: cancels the auto-booking (`_abandon_failed_start_booking`) or reverts preheat to `scheduled` (`_revert_failed_preheat`), ends the Live Activity with reason `start-failed`, records `_start_failure`, and sends a time-sensitive alert to the member *and* the admins. A failure whose activation generation is stale (i.e. someone hit Off) is a cancellation, not a failure — no alert.
- `/api/sauna/status` carries `startFailure: {message, at}` for 15 min after an unconfirmed start (suppressed once `active=1`, cleared by a successful start or any turn-off). The frontend watches it on every status load, so the "starting…" state resolves to a red "Sauna did not start" banner even if the push is missed. **If you add another start path, give it the same unwind — never let a start command report success it hasn't confirmed.**
...
- **Live Activities — one per device, self-ticking countdown**: content-state carries `endsAtMillis`; the widget renders `Text(timerInterval:)` so remaining time counts down on-device even if update pushes stop. Live Activity + push-to-start token uploads include `apnsEnvironment` (build env) so the per-device APNs host can never go stale — a stale env means `BadDeviceToken`, the server dropping the token, and a frozen activity. A remote start (`_fanout_live_activity_start`) first `end`-pushes any existing activities on the target devices (push-to-start always creates a NEW activity); the app also ends all other activities on start/adopt/end. The 60s update job and end fanout fall back to push-to-start tokens (input-push-token semantics) for devices that never uploaded a per-activity token (activity remote-started while app closed). Tapping the Live Activity opens `sweatbox://open`, which maps to the web root; the server also serves the SPA at `/open` for older app builds.
