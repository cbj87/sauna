...
- [ ] **Booking modal time picker has no validation** — entering a past time gives a confusing backend error ("end_time must be after start_time") with no inline hint; entering an hour > 23 silently wraps to the next day; show "Start time is in the past" / "Invalid time" inline before the user submits
- [ ] **Extend button has no upper limit** — +15 min can be tapped indefinitely, extending a session for hours past the original booking; cap at 2 hours beyond the original end time or require admin for additional extends
